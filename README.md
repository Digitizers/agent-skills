...
| [`codex-review-loop`](skills/codex-review-loop/) | Drive a PR to convergence through the Codex AI reviewer — build → local Round 0 pre-review (Codex plugin) → PR → `@codex review` → verify each finding against HEAD → fix the real in-scope ones with regression tests → re-trigger until Codex is clean, or until every blocking finding left is triaged into a terminal state → human reviews last. |
...
| [`pr-first-workflow`](skills/pr-first-workflow/) | Default to a branch + PR for every change (code and docs); branch → commit → local pre-review (Round 0) → push → PR → review → merge → return to the default branch; direct-to-main only on an explicit OK. Pairs with `codex-review-loop`. |
...
| [`ui-component-router`](skills/ui-component-router/) | Router for sourcing UI components — one decision among the shadcn/ui registry (MCP or `npx shadcn add`), 21st.dev magic-mcp (metered, sections/community search), and Ant Design; an explicitly chosen source wins (naming candidates in a question selects nothing), then the existing project stack, one system per project, free before metered, availability probed before routing. Pairs with `design-dna` when a client DESIGN.md constrains theming. |
