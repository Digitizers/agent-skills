...
| [`codex-review-loop`](skills/codex-review-loop/) | Drive a PR to convergence through the Codex AI reviewer — build → local Round 0 pre-review (Codex plugin) → PR → `@codex review` → verify each finding against HEAD → fix the real ones with regression tests → re-trigger until clean → human reviews last. |
